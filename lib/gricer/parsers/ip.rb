module Gricer
  module Parsers
    class Ip
      ...
      # return [Array]
      def self.two_part_domain_endings
        [
      ...
      def self.get_info ip 
        ...
        unless ip.blank?
          info[:ip_address_hash] = Digest::SHA1.hexdigest(ip)

          # IP
          begin
            domain_parts = Resolv.getname(ip).split('.')

            info[:domain] = "#{domain_parts[-1]}"

            info[:domain] = "#{domain_parts[-2]}.#{info[:domain]}" if domain_parts[-2]

            if self.two_part_domain_endings.include? info[:domain]
              info[:domain] = "#{domain_parts[-3]}.#{info[:domain]}"
            end
          rescue
            ip_parts = ip.split('.')
            info[:domain] = "#{ip_parts[0]}.#{ip_parts[1]}.x.x"
          end
          ...
          if ::Gricer.config.geoip_db
            ...
            if geoip 
              info[:country] = geoip[:country_code].try(:downcase)
              info[:region] = geoip[:region].try(:downcase)
              info[:city] = geoip[:city]
              info[:postal_code] = geoip[:postal_code]
              info[:longitude] = geoip[:longitude] 
              info[:latitude] = geoip[:latitude] 
            end
          end
        end
